...
def cifar10(args):
    ...
    trainset.means = (0.5, 0.5, 0.5)
    trainset.stds = (0.5, 0.5, 0.5)
    trainset.bounds = (0, 1)
...
def cifar100(args):
    ...
    test_batch_size = args.test_batch_size if args.batch_size else 128
    num_workers = args.num_workers if args.num_workers else 2
    trainset = CIFAR100(base_path, train=True, transform=transforms.Compose([
                        transforms.ToTensor(),
                        transforms.Normalize((0.4914, 0.4822, 0.4465), (0.2023, 0.1994, 0.2010))]))
    trainset.means = (0.4914, 0.4822, 0.4465)
    trainset.stds = (0.2023, 0.1994, 0.2010)
    trainset.bounds = (0, 1)
